...
### 2. 백엔드 개발 (기여도: 60%)
- **인증/인가 시스템:** Spring Security + OAuth2 통합 구현
- **결제 시스템:** Toss Payments API 연동 및 포인트 시스템 구축  
- **알림 시스템:** Event-driven 아키텍처 기반 실시간 알림 구현
- **AI 챗봇:** Spring AI + OpenAI API 통합 개발

### 3. 프론트엔드 개발 (기여도: 40%)
- **공통 컴포넌트:** 재사용 가능한 UI 컴포넌트 라이브러리 구축
- **상태 관리:** React Context API 기반 전역 상태 관리 구조 설계
- **API 통신:** Axios 기반 HTTP 클라이언트 및 에러 핸들링 표준화
- **반응형 디자인:** Tailwind CSS 활용한 모바일 퍼스트 UI 구현
