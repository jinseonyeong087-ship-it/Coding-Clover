...
import {
  ArrowLeft, User, Calendar, MessageCircle, MoreHorizontal,
...
const StudentQnaDetail = () => {
  ...
  const fetchQnaDetail = async () => {
    try {
      const res = await axios.get(`/student/qna/${qnaId}`);
      setQna(res.data);
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.post(`/student/qna/${qnaId}/delete`, {
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
      await axios.post(`/instructor/qna/${qnaId}/add`, {
        instructorId: currentUser.userId || currentUser.id,
        content: answerContent
      });
      alert("답변이 등록되었습니다.");
      setAnswerContent('');
      fetchQnaDetail(); // Refresh
    } catch (err) {
  ...
  const isOwner = currentUser && (currentUser.userId || currentUser.id) === qna.userId;
            ...
            {(isOwner || isAdmin) && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {/* Edit is complex due to modal requirement, simple delete for now */}
                  <DropdownMenuItem className="text-red-600 cursor-pointer" onClick={handleDelete}>
                    삭제하기
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        <div className="space-y-6">
          ...
          {qna.answers && qna.answers.length > 0 ? (
            qna.answers.map((answer) => (
              <div key={answer.answerId} className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  {/* Action buttons could go here (edit/delete answer) */}
                </div>
            ...
            <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-200">
              ...
              <p className="text-sm text-gray-400">강사님의 답변을 기다려주세요!</p>
            </div>
          )}
