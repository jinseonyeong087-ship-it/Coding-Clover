...
function AdminMain() {
    ...
    useEffect(() => {
        // 강좌 목록 조회
        fetch('/admin/course', { credentials: 'include' })
            .then((response) => response.json())
            .then((resData) => {
                if (Array.isArray(resData)) {
                    setCourse(resData);
                    console.log("데이터 로드 성공", resData);
        ...
        // 강사 목록 조회
        fetch('/admin/users/instructors', { credentials: 'include' })
            .then((response) => response.json())
            .then((data) => {
                setStatus(data);
            })
                                ...
                                status.map((users, index) => {
                                    ...
                                    return (
                                        <TableRow key={uniqueKey}>
                                            ...
                                            <TableCell>
                                                <Link to={`/admin/course/${users.userId}/pending`} className="hover:underline">
                                                    {users.name}
                                                </Link>
                                            </TableCell>
                                            <TableCell>
